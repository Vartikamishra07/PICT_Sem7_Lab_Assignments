...
void buildHuffmanTree(const string& text) {
    ...
    unordered_map<char, int> frequencyMap;  // Count the frequency of each character
    for (char ch : text) {
        frequencyMap[ch]++;
    }

    priority_queue<  HuffmanNode*, vector<HuffmanNode*>, Compare> pq; // Create a priority queue (min-heap)

    for (auto pair : frequencyMap) {
        pq.push(new HuffmanNode(pair.first, pair.second)); // Create a leaf node for each character and add it to the priority queue
    }

    // Iterate until there is only one node left in the priority queue
    while (pq.size() > 1) {
        HuffmanNode* left = pq.top();  // Remove two nodes of the highest priority (lowest frequency)
        pq.pop();
